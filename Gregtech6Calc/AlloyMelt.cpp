...
heatCalc::heatCalc()
{
	...
	ChamberAbout.id = 0, CrucibleAbout.id = 0, ItemAbout.id[0] = 0;
...
u16 heatCalc::getItem(u16 id, u16 n, u16 pos, u16 mode)        //get each item id and number, mode 0:setup, 1:setup in which, 2:back to past point 3:end typing
{
	...
	case 0:          //setup and go next
		ItemAbout.id[ItemAbout.kind] = id;
...
{
	eta = ChamberAbout.pos[ChamberAbout.id].effectiveness;
	Power0 = ChamberAbout.pos[ChamberAbout.id].HUoutput;
	if (ChamberAbout.dense)
	{
		Power0 *= 4;
	}
	Q0 = T * m / 100;
	...
	//if got any 0 or inf, plz check wheather writed all information
}